...
function render() {
  ...
  if (currentView === "timetable") return renderTimetable();
  if (currentView === "todo") return renderTodo();
...
function renderHome() {
  ...
  viewEl.innerHTML = `
    ...
    <div class="section-title">오늘 시간표</div>
    <div id="tt-slot"></div>
  `;
...
function renderMealSlot(ymd) {
  ...
  fetchMeal(ymd)
    ...
    .catch((err) => {
      const el = document.getElementById("meal-text");
      if (!el) return;
      el.innerHTML = `급식 정보를 불러오지 못했어요.<br><span style="font-size:11.5px;color:var(--chalk)">${escapeHtml(err.message || "")}</span>`;
    });
}
...
function renderTodo() {
  ...
  viewEl.innerHTML = `
    <div class="section-title" style="margin-top:8px">할일</div>
    <div class="add-row">
...
function renderNotes() {
  ...
  viewEl.innerHTML = `
    <div class="section-title" style="margin-top:8px">노트</div>
    <div class="add-row">
